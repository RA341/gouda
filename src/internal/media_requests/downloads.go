...
type DownloadService struct {
	db     *gorm.DB
	client download_clients.DownloadClient
	// run status check only the first time
	// statuscheck will handle checks until all downloads are complete
	once sync.Once
}

func NewDownloadService(db *gorm.DB, client download_clients.DownloadClient) *DownloadService {
...
func (d *DownloadService) SaveTorrentReq(request *RequestTorrent) error {
...
func (d *DownloadService) AddTorrent(request *RequestTorrent, torrentFilePath string) error {
	if d.client == nil {
...
func (d *DownloadService) GetTorrentFileLocation(request *RequestTorrent, downloadFile bool) (string, error) {
...
func (d *DownloadService) CheckStatus(statuses []download_clients.TorrentStatus, ignoreTimeout bool, torrentCheckTimeout time.Duration) {
	for _, torrentStatus := range statuses {
		var requestInfo RequestTorrent
...
func (d *DownloadService) finalizeDownload(torrentRequest *RequestTorrent, torrentStatus *download_clients.TorrentStatus) error {
...
func (d *DownloadService) fetchDownloadingTorrentsIds() ([]string, error) {
	...
	result := d.db.Model(&RequestTorrent{}).
		Where("status = ?", "downloading").
...
func (d *DownloadService) SetClient(client download_clients.DownloadClient) {
	d.client = client
}
